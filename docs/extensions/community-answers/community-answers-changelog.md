---
sidebar_position: 100
---

# Community Answers Changelog

All notable changes to Community Answers will be documented in this file.


## [7.0.1] - 2026-06-27

### 🚀 Added
- Added Rewardify adapter plugin

### 🐛 Fixed
- Fixed issue preventing bulk delete of trashed questions
- Fixed error when permanently deleting questions
- Fixed issue where package did not remove modules and plugins

## [7.0.1] - 2026-06-27

### 🚀 Added
- Added Rewardify adapter plugin

### 🐛 Fixed
- Fixed bulk delete trashed questions issue
- Fixed error on permanent question deletion
- Fixed package unable to remove modules and plugins

## [7.0.1] - 2026-06-27

### 🚀 Added
- Added Rewardify adapter plugin

### 🐛 Fixed
- Fixed package unable to remove modules and plugins

## [7.0.0] - 2026-06-08

### 🚀 Added
- Added new option to show login form to the guest users

### 🐛 Fixed
- Fixed issue with Joomla 6 installation
- Fixed issue with backend answers page when filtered by category * Fixed issue with comments display
- Fixed issue with indexing question in smart search plugin

## [7.0.0] - 2026-06-08

### 🚀 Added
- Added new option to show login form to the guest users

### 🐛 Fixed
- Fixed issue with Joomla 6 installation
- Fixed issue with backend answers page when filtered by category * Fixed issue with comments display
- Fixed issue with indexing question in smart search plugin
